...
- `web/packages/providers/src/providers/aniwave.ts` must probe all `/ajax/server/list` candidates instead of trusting the first `data-link-id`, and `web/apps/api/src/services/relay-service.ts` is the companion file to check when old sessions keep redirecting to dead `shipimagesbolt.online/embed-1/...` pages. Use those two files first when Aniwave says a file was deleted while the same episode still works on-site, because Vidplay is often dead while MyCloud/Cloudora from the same AJAX payload still return live embeds, and the dead Echovideo page can return `200 OK` with a `File not found - Echovideo` title instead of a transport error.
