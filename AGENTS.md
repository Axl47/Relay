...
- `web/packages/providers/src/providers/javguru.ts` now prefers the shortcode button payloads over raw iframe scraping, and it tries to promote resolved hosts like `emturbovid` into direct HLS streams before falling back to iframe embeds. Check it together with `web/apps/api/src/services/relay-service.ts` when Javguru playback falls back to a black screen or `Embeds disabled`, because stale sessions that cached old HTML/embed URLs also need to be treated as non-reusable.
