...
- `web/packages/providers/src/providers/javguru.ts` now prefers the shortcode button payloads over raw iframe scraping, because live Javguru posts often include an ad iframe before the real player. Check it together with `web/apps/api/src/services/relay-service.ts` when Javguru playback falls back to a black screen, because stale sessions that cached `creative.mnaspm.com` or `/searcho/` URLs also need to be treated as non-reusable.
