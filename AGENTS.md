...
- `web/apps/browser/src/extractors/hentaihaven.ts`, `web/apps/browser/src/browser/context-manager.ts`, and `web/apps/api/src/services/relay-service.ts` are the first files to check when HentaiHaven search/details/playback regress, because the extractor depends on the current `/search/<query>/`, `/watch/<slug>/`, and `wp-content/plugins/player-logic/api.php` flow while the site's Cloudflare gate only clears in Relay after the shared browser context presents a normal desktop profile, and stale `text/html` playback sessions need to be treated as non-reusable so Relay does not reopen a blocked iframe after direct HLS extraction starts working again. Re-verify the `.page-item-detail.video` search cards, `.wp-manga-chapter` episode links, the `player_logic_item iframe`, the `api.php` `octopusmanifest.org` response, and HentaiHaven session reuse behavior before changing anything deeper in the stack.
- `web/apps/api/src/app.ts` is the first file to check when HLS playback stalls on proxied `__upstream__` fragment requests even though the manifest itself loads, because some upstream hosts such as HentaiHaven's `fdc.anpustream.com` mislabel binary init fragments like `i.mp4` as `application/x-mpegURL`. Keep HLS playlist rewriting path-aware there so Relay only rewrites real `.m3u8` responses and does not corrupt mislabelled MP4 fragments into text.
