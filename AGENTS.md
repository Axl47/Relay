...
- `web/apps/browser/src/extractors/animeonsen.ts` is the first file to check when AnimeOnsen playback breaks, because the site’s watch page hides the actual manifest behind `https://api.animeonsen.xyz/v4/content/<ao-content-id>/video/<episode>` and the extractor has to derive the bearer token from the browser-visible `ao.session` cookie before reading `data.uri.stream` / subtitle URLs. Re-verify the `meta[name="ao-content-id"]` extraction, the `ao.session` token heuristic, and the episode selector parsing before assuming the stream host itself changed.
